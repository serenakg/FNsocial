...
- **"Recognition from Josie" and event sponsorship** (`investors.html`, Traction section): still open. Serena confirmed she'll supply Josie's full name/title, and that this should appear both on `investors.html` (the existing `.stat-pending` "Recognition & sponsorship details to confirm" spot) and as a quote from her letter on the homepage. Her "message of support" letter (for the Feb 2026 launch event) has been extracted but not yet placed — it currently has no signature/title in its source text, so it's held pending Serena's confirmation of how to credit her. See her letter content in the session's scratchpad if picking this back up.
