...
- **Investor market sizing** (TAM/SAM/SOM, `investors.html`): built with real figures from Serena's own pricing model, then removed entirely per her later dev brief ("we don't need all this, remove it"). The 22%/77%/700M stats-grid above where it sat is unaffected.
...
- **Partner form** (`about.html`, "Partner with us") and **Employer expression-of-interest form** (`about.html` + the homepage role funnel): resolved — both now link to real Google Forms Serena supplied, replacing the earlier `mailto:` fallbacks.
